...
public:
    ...
    /* 检测人脸 */
    int detect_face(const cv::Mat& src, std::vector<cv::Rect>& outFaces);

    /* 反初始化检测器*/
    int UnInitDetector();

    // pnet 分层计算
    int RunPNetLayer(const cv::Mat& src, int scale_idx, std::vector<float>& outBoxes, std::vector<float>& outScores);

    int PrePNET(const cv::Mat& src, float scalor, std::vector<torch::jit::IValue>& ti);

    int PostPNET(torch::jit::IValue& to, float scalor, std::vector<float>& outBoxes, std::vector<float>& outScores);

protected:
    int PNET(const cv::Mat& src, std::vector<cv::Rect>& outFaces);
    int RNET(const cv::Mat& src, std::vector<cv::Rect>& outFaces);
    int ONET(const cv::Mat& src, std::vector<cv::Rect>& outFaces);
    ...
    //rnet onet的共用
    int RunRONet(torch::jit::script::Module net, const cv::Mat& src, std::vector<cv::Rect>& outFaces,
                 int IMAGE_SIZE);
